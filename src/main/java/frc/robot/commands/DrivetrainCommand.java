...
public class DrivetrainCommand extends Command {
    ...
    private final double MaxSpeed = TunerConstants.kSpeedAt12Volts.in(MetersPerSecond); // Top speed desired at 12v
    private final double MaxAngularRate = RotationsPerSecond.of(0.75).in(RadiansPerSecond); // 3/4 Rotations per second max
    private final SwerveRequest.FieldCentric drive = new LegacySwerveRequest.FieldCentric().withDeadband(MaxSpeed * 0.25).withRotationalDeadband(MaxAngularRate * 0.25).withDriveRequestType(SwerveModule.DriveRequestType.OpenLoopVoltage);
            ...
            switch(pose) {
                case TELEOP:
                    subsystem.setControl(
                        drive
                            .withVelocityX(
                                -leftY.getAsDouble() * MaxSpeed) // Forward with negative Y (Forward)
                            .withVelocityY(
                                -leftX.getAsDouble() * MaxSpeed) // Drive left with negative X (Left)
                            .withRotationalRate(
                                -rightX.getAsDouble()
                                * MaxAngularRate));
